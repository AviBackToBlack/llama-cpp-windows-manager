...
public sealed class RuntimeAdapterTests
{
    ...
    [Fact]
    public void ValidateRejectsQuotedHostTokenInExtraArgs()
    {
        // A quoted custom parameter ("--host 0.0.0.0") parses to a single token containing a
        // space; the security-critical check must still catch it instead of letting it fail
        // later as an opaque llama-server error.
        var result = RuntimeAdapter.Validate(ValidRequest() with
        {
            ExtraArgs = CustomLaunchParameterParser.Parse("\"--host 0.0.0.0\"")
        });
    ...
    [Fact]
    public void ValidateRejectsPortAndApiKeyFlagsInExtraArgs()
    {
        var result = RuntimeAdapter.Validate(ValidRequest() with
        {
            ExtraArgs = CustomLaunchParameterParser.Parse(@"--port 9090 --api-key secret --api-key-file C:\keys.txt")
        });
        ...
        Assert.Contains(result.Errors, e => e.Contains("--api-key-file", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void BuildArgsRoundTripsDefaultSettings()
    {
        ...
        try
        {
            ...
            // --ctx-size 0 means "load context size from the model" and must survive the
            // round trip; omitting it would hand the decision to the server's own default.
