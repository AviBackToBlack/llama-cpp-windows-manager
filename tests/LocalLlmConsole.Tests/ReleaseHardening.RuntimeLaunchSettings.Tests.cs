...
public sealed partial class ReleaseHardeningTests
{
    ...
    public void ModelLaunchProfilePersistenceIsExplicit()
    {
        ...
        Assert.Contains("_coreServices.Models.ModelLaunchHeadSelectionApplication.ChooseDraftModel(", source, StringComparison.Ordinal);
        ...
        Assert.Contains("var profile = await _profiles.ReadAsync(model, profileId);", workflow, StringComparison.Ordinal);
        Assert.Contains("var effective = profile ?? await _profiles.DraftAsync(model, defaults, profileId);", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("actions.MarkSaved(request.ModelId, request.ProfileId, request.Result.SavedSettings)", saveApplication, StringComparison.Ordinal);
    ...
    public async Task ModelLaunchSettingsSaveApplicationServiceAppliesProfileAndDefaultSavesInOrder()
    {
        ...
        service.ApplyProfileSave(
            ...
            new ModelLaunchProfileSaveActions(
                (modelId, _, profile) => calls.Add($"mark:{modelId}:{profile.RuntimeId}"),
    ...
    public async Task ModelLaunchVariantSaveApplicationServiceAppliesFailureAndSuccessFollowupsInOrder()
    {
        ...
        var profile = new NamedModelLaunchProfile("profile-1", source.Id, "Qwen 32K", ModelLaunchSettings.FromAppSettings(settings), DateTimeOffset.UtcNow);
        ...
        var succeeded = await service.ApplyAsync(
            new ModelLaunchVariantSaveApplicationRequest(new ModelLaunchVariantWorkflowResult(true, "Saved.", profile), settings),
            Actions());

        Assert.True(succeeded);
        Assert.Equal(["refresh-models", "select:profile-1", "render", "overview", $"opencode:{settings.WorkspaceRoot}", "status:Saved."], calls);

        calls.Clear();
        var succeededWithoutSync = await service.ApplyAsync(
            new ModelLaunchVariantSaveApplicationRequest(
                new ModelLaunchVariantWorkflowResult(true, "Saved.", profile),
                settings with { AutoSaveOpenCodeOnLaunchSettingsSave = false }),
            Actions());

        Assert.True(succeededWithoutSync);
        Assert.Equal(["refresh-models", "select:profile-1", "render", "overview", "status:Saved."], calls);
        ...
        var noModel = await service.SaveSelectedAsNewAsync(
            ...
            "Qwen 32K",
            settings,
            SaveSelectedVariantActions(editorLoaded: true, new ModelLaunchVariantWorkflowResult(true, "Saved.", profile)));
        Assert.Equal(ModelLaunchVariantSaveApplicationOutcome.NoModelSelected, noModel);
        Assert.Equal(["status:Select a model before saving a named launch profile."], calls);
        ...
        var selected = await service.SaveSelectedAsNewAsync(
            ...
            "Qwen 32K",
            settings,
            SaveSelectedVariantActions(editorLoaded: false, new ModelLaunchVariantWorkflowResult(true, "Saved.", profile)));
        Assert.Equal(ModelLaunchVariantSaveApplicationOutcome.Saved, selected);
        Assert.Equal(
            [
                "busy:Saving named launch profile...",
