...
public sealed class RuntimeFlagTests
{
    ...
    [Theory]
    [InlineData("on", "--flash-attn", null)]
    [InlineData("off", null, "--no-flash-attn")]
    [InlineData("auto", null, null)]
    public void BuildCommand_EmitsFlashAttentionBooleanCorrectly(string value, string? positiveToken, string? negativeToken)
    {
        ...
        if (positiveToken is not null)
            Assert.Contains(positiveToken, tokens);
        if (negativeToken is not null)
            Assert.Contains(negativeToken, tokens);
        if (positiveToken is null && negativeToken is null)
        {
