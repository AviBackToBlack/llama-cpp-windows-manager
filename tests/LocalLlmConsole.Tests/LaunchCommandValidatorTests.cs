...
public sealed class LaunchCommandValidatorTests
{
    ...
    [InlineData("--api-key-file", @"C:\keys.txt")]
    ...
    [Fact]
    public void UnknownFlagIsTolerated()
    {
        // Persistence deliberately preserves schema-unknown flags so saved profiles survive
        // schema changes across app versions; rejecting them would make such a profile
        // unlaunchable with no UI control to clear the stale entry.
        var flags = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["--unknown-flag"] = "value"
        };
