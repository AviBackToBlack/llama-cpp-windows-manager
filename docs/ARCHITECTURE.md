...
12. Optionally keep OpenCode local model entries synced after saved model launch settings or variants change, including gateway/direct endpoint selection, `limit.output` derived from the model's Max tokens launch setting, and vision support metadata.
