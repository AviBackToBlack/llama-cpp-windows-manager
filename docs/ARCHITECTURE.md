...
- `LlamaServerFlagSchema`, `LaunchCommandService`, `LaunchCommandValidator`, `RuntimeFlagHelpParser`, `RuntimeFlagCapabilityService`, `RuntimeFlagHelpRunner`, `RuntimeAdapter`, `LaunchSettingMetadataService`, `LaunchSettingsControlFactory`, `LaunchSettingsPanelFactory`, `LaunchSettingsFormBinder`, `LaunchSettingsPanelState`, `LaunchSettingsControlStateService`, `LaunchSettingsRuntimeCapabilityApplicationService`, and `LaunchSettingsRenderApplicationService` own the schema-driven launch settings catalog, command line builder/parser, validation, runtime `--help` capability detection, command preview, generated WPF controls, and launch settings UI state. Domain-level launch settings (`RuntimeLaunchRequest` and `LlamaServerLaunchOptions`) are validated by `RuntimeAdapter` before execution; `--host`, `--port`, and `--api-key` are never exposed to user overrides.
...
3. Pick a prebuilt or custom built llama.cpp runtime and launch settings. The launch settings panel is generated from `LlamaServerFlagSchema`, so it exposes every known `llama-server` flag. First-class controls show the most common flags; remaining flags are generated into categorized sections when Advanced is enabled. A live two-way command preview updates the form when a command is pasted and updates the command when the form changes. `RuntimeFlagCapabilityService` parses the selected runtime's `--help` output and disables flags that runtime does not advertise. `LaunchCommandValidator` rejects out-of-range values, unknown enum values, missing file/path entries, and invalid cross-field rules before save or launch. `--host`, `--port`, and `--api-key` are never accepted from the command preview or Custom params.
