...
public sealed class SettingsPageDefinitionService
{
    ...
    [
        ...
        new("OpenCode", "Auto-sync entries", "autoSaveOpenCodeOnLaunchSettingsSave", AppPreferenceService.YesNoLabel(settings.AutoSaveOpenCodeOnLaunchSettingsSave), "choice", AppPreferenceService.YesNoOptions(),
        ...
        new("OpenCode", "Limit output", "openCodeOutputLimit", settings.OpenCodeOutputLimit.ToString(CultureInfo.InvariantCulture),
            ToolTip: $"Whole number from 1 to {AppSettings.MaxOpenCodeOutputLimit.ToString(CultureInfo.InvariantCulture)}. Written to OpenCode local model entries as limit.output when OpenCode settings are saved or synced."),
