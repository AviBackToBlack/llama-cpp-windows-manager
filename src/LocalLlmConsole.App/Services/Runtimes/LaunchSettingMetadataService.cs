...
public static class LaunchSettingMetadataService
{
    ...
    public static string Tooltip(string label) => label switch
    {
        ...
        "Vision" => "Enables image input support when the selected model has embedded vision or a matching mmproj/projector file.",
        "Vision head" => "Chooses the vision source: auto-detect a nearby companion, use embedded/model-bundled vision, or pick an external mmproj/projector GGUF.",
        ...
        "Prompt cache" => "Controls llama.cpp's in-memory prompt cache. Auto keeps the runtime default.",
        "Prompt cache MB" => "Maximum prompt cache RAM in MiB when Prompt cache is on. -1 removes the limit.",
        "Checkpoints" => "Controls context checkpoints used to reuse long prompt prefixes. Auto keeps the runtime default.",
        "Checkpoint count" => "Maximum context checkpoints per slot when checkpoints are on.",
        "Checkpoint spacing" => "Create a context checkpoint every N prompt tokens when checkpoints are on.",
        ...
        "Spec type" => "Speculative decoding mode. atomic-mtp emits --mtp-head for Atomic Gemma MTP forks; draft-mtp uses upstream llama.cpp's draft path when supported.",
        ...
        "MTP head" => "Optional MTP assistant/head GGUF for Atomic runtimes that accept --mtp-head. Leave blank to auto-detect a nearby MTP file when Spec type is atomic-mtp.",
