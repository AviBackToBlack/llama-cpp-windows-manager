...
public static class RuntimeAdapter
{
    ...
    private static void ValidateExtraArgs(IReadOnlyList<string> extraArgs, List<string> errors)
    {
        ...
        foreach (var arg in extraArgs)
        {
            ...
            // A quoted custom parameter like "--host 0.0.0.0" arrives as one token containing
            // a space, so the flag name must be cut at the first '=' or whitespace to keep it
            // from slipping past the security-critical check with an opaque server error.
            var separator = arg.IndexOfAny(['=', ' ', '\t']);
            var flagName = separator > 0 ? arg[..separator] : arg;
            if (LlamaServerFlagSchema.FindByName(flagName)?.IsSecurityCritical == true)
            {
    ...
    public static IReadOnlyList<string> BuildArgs(RuntimeLaunchRequest request)
        => BuildArgs(request, droppedFlags: null);

    /// <param name="droppedFlags">
    /// When set, receives flags omitted because the selected runtime does not advertise them,
    /// so the caller can log the divergence from the command preview.
    /// </param>
    public static IReadOnlyList<string> BuildArgs(RuntimeLaunchRequest request, ICollection<string>? droppedFlags)
    {
        var validation = Validate(request);
        if (!validation.Ok) throw new InvalidOperationException(string.Join(" ", validation.Errors));

        var host = NormalizeHost(request.Host);
        var options = LlamaServerLaunchOptions.From(request, host);

        var args = LaunchCommandService.BuildCommandTokens(options, droppedFlags).ToList();
        args.InsertRange(0, ["--host", host, "--port", request.Port.ToString(System.Globalization.CultureInfo.InvariantCulture)]);
        var extraArgs = request.ExtraArgs.Where(arg => !string.IsNullOrWhiteSpace(arg)).ToList();
        args.AddRange(LaunchCommandService.FilterExtraArgsForRuntime(extraArgs, request.SupportedFlags, droppedFlags));
        return args;
    }
