...
public sealed partial class HuggingFaceService
{
    ...
    private async Task<RepoInfo> GetRepoInfoAsync(string repo, JsonNode? fallback, CancellationToken cancellationToken)
    {
        ...
        if (_repoInfoCache.TryGetValue(repo, out var cached))
        {
            if (now - cached.CachedAt <= RepoInfoCacheTtl) return cached.Info;
            _repoInfoCache.TryRemove(repo, out _);
        }
        ...
        var info = new RepoInfo(
            ...
            projector?.Sha256 ?? "",
            detail);
        _repoInfoCache[repo] = new CachedRepoInfo(info, now);
        TrimRepoInfoCache(now);
        return info;
    }

    private void TrimRepoInfoCache(DateTimeOffset now)
    {
        foreach (var stale in _repoInfoCache
            .Where(entry => now - entry.Value.CachedAt > RepoInfoCacheTtl)
            .Select(entry => entry.Key)
            .ToArray())
            _repoInfoCache.TryRemove(stale, out _);

        var overflow = _repoInfoCache.Count - RepoInfoCacheLimit;
        if (overflow <= 0) return;

        foreach (var key in _repoInfoCache
            .OrderBy(entry => entry.Value.CachedAt)
            .Take(overflow)
            .Select(entry => entry.Key)
            .ToArray())
            _repoInfoCache.TryRemove(key, out _);
    }
