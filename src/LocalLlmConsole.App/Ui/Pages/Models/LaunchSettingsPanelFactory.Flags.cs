...
public static partial class LaunchSettingsPanelFactory
{
        ...
        Func<LlamaServerFlag, bool>? skip = null)
    {
        var flags = LlamaServerFlagSchema.All.Where(f =>
            ...
            !f.IsSecurityCritical &&
            !IsAppManagedFlag(f.PrimaryName) &&
            !IsNegatedAliasFlag(f) &&
    ...
    private static bool IsNegatedAliasFlag(LlamaServerFlag flag)
    ...
    private static bool IsAppManagedFlag(string primaryName)
        => string.Equals(primaryName, "--model", StringComparison.OrdinalIgnoreCase);
}
